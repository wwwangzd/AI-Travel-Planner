...
**1. 偏好设置**
- 兴趣偏好（interests）：数组形式存储，如 ["美食", "动漫", "历史文化"]
- 特殊需求（specialNeeds）：数组形式存储，如 ["带孩子", "带老人", "无障碍需求"]

**2. 偏好应用**
- 新建计划时，系统自动读取用户偏好并填充到 preferences 字段
- 用户可以在创建计划时修改或覆盖默认偏好
- 修改后的偏好仅应用于当前计划，不影响用户默认偏好

**3. 偏好学习机制**
- 定期分析用户历史计划中的 preferences 字段
- 统计出现频率最高的 interests 和 specialNeeds
- 自动更新用户默认偏好（可选功能）

**4. 数据存储**
- 用户表中存储默认偏好（interests 和 specialNeeds 数组）
- 每个旅行计划中也保存当次使用的偏好
- 两者独立存储，互不影响
